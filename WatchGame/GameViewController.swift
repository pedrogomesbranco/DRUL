...
class GameViewController: UIViewController {
    ...
    @IBAction func handleSwipe(swipeGesture: UISwipeGestureRecognizer) {
        gameScene.didSwipe(swipeGesture: swipeGesture)
        scoreLabel.text = "\(userRecord.value(forKey: "score")!)"
    }
    
    @IBAction func handleTap(tapGesture: UITapGestureRecognizer) {
        if gameScene.canRestart() {
            scoreLabel.text = "0"
        }
    }
